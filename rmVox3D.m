...
%main idea: define a 3D surface with tickness, the grey levels of the
%voxels located on this 3D surface will be removed
%
%   [vxlst] = rmVox3D(A,SLlst,e)
%   ----------------------------
%
%   Inputs
%   ------
%       >     A : image to be processed
%       > SLlst : list of the slices based on which the 3D surface will be
%                 defined
%       >     e : thickness of the 3D surface
%
%   Outputs
%   -------
%       > vxlst : the list of voxels on the 3D surface
%       
% 2018 Yang Chen
...
for i=SLlst
    SL = A(:,:,i);
    imshow(SL,[min(SL(:)) max(SL(:))]);title(['slice number: ',num2str(i)]);
